...
(function () {
  ...
  function fillMeta() {
    ...
    setTxt("kpi-pontos",       D.kpis.pontosInstalados.toLocaleString("pt-BR"));
    setTxt("kpi-semcad",       D.kpis.semCadastro);
    setTxt("kpi-sc-total",     D.semCadastro.total);
    setTxt("kpi-sc-transm",    D.semCadastro.transmitindo);
    setTxt("kpi-sc-offline",   D.semCadastro.semComunicacao);
    setTxt("kpi-sc-sgi",       D.semCadastro.encontradosSGI);
  }
  ...
  function buildRedes() {
    ...
    newChart("c-redes-bar", {
      type: "bar",
      data: {
        labels: top15.map(r => r[0]),
        datasets: [
          { label: "Cadastrados",  data: top15.map(r => r[2]), backgroundColor: C.blue, borderRadius: 3, barPercentage: .75 },
          { label: "Sem cadastro", data: top15.map(r => r[3]), backgroundColor: C.red,  borderRadius: 3, barPercentage: .75 },
        ],
      },
      options: {
        indexAxis: "y", responsive: true, maintainAspectRatio: false,
        plugins: { legend: { display: true, position: "top", align: "end", labels: { boxWidth: 10, boxHeight: 10, font: { size: 11 }, padding: 12 } }, tooltip: { mode: "index", intersect: false } },
        scales: {
          x: { stacked: true, grid, ticks: tick, border: { display: false } },
          y: { stacked: true, grid: { display: false }, ticks: { ...tick, font: { size: 10 } } },
        },
      },
    });

    /* Sem cadastro por rede */
    const comSC = D.redes.filter(r => r[3] > 0);
    newChart("c-sem-cad", {
      type: "bar",
      data: { labels: comSC.map(r => r[0]), datasets: [{ data: comSC.map(r => r[3]), backgroundColor: comSC.map(r => r[1] === "4G" ? C.blue : C.teal), borderRadius: 4, barPercentage: .7 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: noLeg }, scales: { x: { grid: { display: false }, ticks: tick }, y: { grid, ticks: tick, border: { display: false } } } },
    });

    /* 4G vs Fibra */
    const g4 = D.redes.filter(r => r[1] === "4G").reduce((s, r) => s + r[2] + r[3], 0);
    const fb = D.redes.filter(r => r[1] === "Fibra").reduce((s, r) => s + r[2] + r[3], 0);
    newChart("c-tipo-rede", {
      type: "bar",
      data: { labels: ["4G", "Fibra"], datasets: [{ data: [g4, fb], backgroundColor: [C.blue + "CC", C.teal + "CC"], borderRadius: 5, barPercentage: .6 }] },
      options: { indexAxis: "y", responsive: true, maintainAspectRatio: false, plugins: { legend: noLeg }, scales: { x: { grid, ticks: tick, border: { display: false } }, y: { grid: { display: false }, ticks: tick } } },
    });

    /* Tabela redes */
    appendRows("rede-table", D.redes.map(([rede, tipo, cad, scad]) => {
      const total = cad + scad;
      const tipoPill = tipo === "4G" ? pill("4G", "pill-blue") : tipo === "Fibra" ? pill("Fibra", "pill-teal") : "—";
      let status;
      if (tipo === "—") status = pill("Sem dados", "pill-gray");
      else if (scad > 0) status = pill("+" + scad + " s/ cadastro", "pill-red");
      else if (rede === "REDE 50") status = pill("Nova rede", "pill-amber");
      else status = pill("OK", "pill-green");
      return [
        { v: rede,               cls: "bold" },
        { v: tipoPill },
        { v: cad  || "—",        cls: "r mono" },
        { v: scad || "—",        cls: "r mono" },
        { v: total || "—",       cls: "r mono bold" },
        { v: status },
      ];
    }));
  }
  ...
  function buildCadastro() {
    const sc = D.semCadastro;

    /* Donut distribuição por rede */
    newChart("c-transm-rede", {
      type: "doughnut",
      data: {
        labels: sc.porRede.map(r => r.rede),
        datasets: [{ data: sc.porRede.map(r => r.transmitindo + r.semComm), backgroundColor: [C.blue, C.blue2, C.teal, C.teal2, "#0E7490", C.gray], borderWidth: 0, hoverOffset: 6 }],
      },
      options: { responsive: true, maintainAspectRatio: false, cutout: "55%", plugins: { legend: { display: true, position: "right", align: "center", labels: { boxWidth: 10, boxHeight: 10, font: { size: 10 }, padding: 8 } } } },
    });

    /* Bairros (SGI) */
    hbars("bairro-sc-bars", sc.porBairro.map(b => [b.bairro, b.qty]), C.amber);

    /* Tabela por rede */
    const totalT = sc.porRede.reduce((s, r) => s + r.transmitindo, 0);
    const totalS = sc.porRede.reduce((s, r) => s + r.semComm, 0);
    const totalA = totalT + totalS;

    appendRows("sc-table", sc.porRede.map(r => {
      const total = r.transmitindo + r.semComm;
      const pct   = Math.round(total / sc.total * 100);
      return [
        { v: r.rede,         cls: "bold" },
        { v: r.transmitindo, cls: "r mono" },
        { v: r.semComm,      cls: "r mono" },
        { v: total,          cls: "r mono bold" },
        { v: `<div class="inline-track"><div class="inline-fill" style="background:${r.tipo==="4G"?C.blue:r.tipo==="Fibra"?C.teal:C.gray};width:${pct}%"></div></div>${pct}%` },
      ];
    }));

    /* Total row */
    const tbody = $("sc-table");
    if (tbody) {
      const tr = document.createElement("tr");
      tr.className = "total-row";
      tr.innerHTML = `<td class="bold">TOTAL</td><td class="r mono">${totalT}</td><td class="r mono">${totalS}</td><td class="r mono bold">${totalA}</td><td>100%</td>`;
      tbody.appendChild(tr);
    }
  }
